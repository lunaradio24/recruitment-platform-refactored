...
resumeRouter.patch('/:resumeId', requireRoles(['APPLICANT']), updateResumeValidator, async (req, res, next) => {
  try {
    ...
    // 4. 유효성 검증 및 에러 처리
    if (!existingResume) throw new HttpError.NotFound(MESSAGES.RESUMES.COMMON.NOT_FOUND);
...
/*****     이력서 지원 상태 변경 API     *****/
resumeRouter.patch('/:resumeId/status', requireRoles(['RECRUITER']), updateStatusValidator, async (req, res, next) => {
  try {
    // 1. 사용자 정보와 요청 정보를 가져오기
