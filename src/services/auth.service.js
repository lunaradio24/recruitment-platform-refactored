...
export class AuthService {
  ...
  signUp = async (email, password, name) => {
    ...
    const createdUser = await this.userRepository.createUser(email, hashedPassword, name);
  ...
  signIn = async (email, password) => {
    ...
    const existingToken = await this.tokenRepository.findRefreshTokenByUserId(user.id);

    // 없다면(없다면) 새로 발급한 Refresh Token을(으로) DB에 저장(갱신)
    if (!existingToken) await this.tokenRepository.createRefreshToken(user.id, hashedToken);
    else await this.tokenRepository.updateRefreshToken(user.id, hashedToken);
  ...
  signOut = async (userId) => {
    ...
    await this.tokenRepository.updateRefreshToken(userId, null);
  };
  ...
  renewTokens = async (userId) => {
    ...
    await this.tokenRepository.updateRefreshToken(userId, saltedToken);
